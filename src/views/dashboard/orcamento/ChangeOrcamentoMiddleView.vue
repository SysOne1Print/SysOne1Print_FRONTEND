      ...
      >
        <div class="flex d-flex flex-column flex-sm-row align-items-center mb-24pt mb-md-0">
          <div class="mb-24pt mb-sm-0 mr-sm-24pt">
            <h2 class="mb-0">
              Alterar Orçamento - id {{ productStore.budgetStateComputed?.orc_id }}
            </h2>

            <ol class="breadcrumb p-0 m-0" data-v-e3707588="">
              <!--&lt;li class=&quot;breadcrumb-item&quot;&gt;&lt;a href=&quot;defCliente.html&quot;&gt;Pedidos&lt;/a&gt;&lt;/li&gt;-->
              <li class="breadcrumb-item active" data-v-e3707588="">Alterar Orcamento</li>
            </ol>
          </div>
        </div>
...
import { getClientByid } from '@/api/actions/Client'
...
const { setClientById } = useClientStore()
...
onMounted(async () => {
  ...
  try {
    clientState.value = []
    const id_orc: number = Number(router.currentRoute.value.params.id_orc)
    const id_cli: number = Number(router.currentRoute.value.params.id_cli)
    const { data } = await getClientByid(id_cli)
    clientState.value.push(data)
    setClientById(id_cli)
    await Promise.allSettled([
      productStore.getAllProductStore(limit.value),
      productStore.getBudgetChangeByIdStore(id_orc)
    ])
  } catch (er: unknown) {
    ...
    notifyToast({ msg: 'Erro ao buscar dados do orçamento', typeToast: 'error' })
  }
})
